...
def run_spacer_blast(InputSpacerFile,OutputSpacerFile,BlastMode,ProkDatabase,blastThreads):
    ProkDatabase = os.path.abspath(ProkDatabase)
 ...
 if indb == 'phage':
    ...
    '''
    if BlastMode == 'common':
        cmd = "blastn -query "+InputSpacerFile+" -db "+ProkDatabase+" -out "+OutputSpacerFile+" -outfmt '6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qlen' -word_size 16 -gapopen 1 -gapextend 1 -penalty -2 -reward 1 -task blastn-short -num_threads "+str(blastThreads)
        os.system(cmd)
    elif BlastMode == 'relax':
        cmd = "blastn -query "+InputSpacerFile+" -db "+ProkDatabase+" -out "+OutputSpacerFile+" -outfmt '6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qlen' -word_size 7 -gapopen 5 -gapextend 2 -penalty -1 -reward 1 -task blastn-short -num_threads "+str(blastThreads)
        os.system(cmd)
    else:
        print("Wrong blast mode, please check.")
        return False
...
def get_significant_blastp_hits(blastres,significantres,incov,inident):

    with open(blastres,'r') as fa, open(significantres,'w') as fb:
        for faline in fa.readlines():
            pident = float(float(faline.strip().split("\t")[2])/100)
            qcovs = float(int(faline.strip().split("\t")[-3])/100)
            if pident >= inident and qcovs >= incov:
                fb.write(faline)
...
def revise_spacer_file_LOCAL(infile,bacDB,repeatSeq,inrident, rawspacerfile, outfile):
    # PAMPHLETResources.revise_spacer_file_LOCAL(SignificantHitResult, bacDB, repeatseq, inrident, RevisedSpacerFile)
    operonCollectionFile = os.path.join(bacDB,"operonCollection.pkl")
    spacerCollectionFile = os.path.join(bacDB,"spacerCollection.pkl")
    operonCollection = pkl.load(open(operonCollectionFile,'rb'))
    spacerCollection = pkl.load(open(spacerCollectionFile,'rb'))
    acceptCRISPR = []
    acceptCRISPRorientation = {}
    with open(infile,'r') as fa:
        ### This is blast result
        for faline in fa.readlines():
            sseqid = faline.strip().split("\t")[1]
            operonID = sseqid.split("#")[1]
            if operonID in operonCollection:
                crisprIDs = operonCollection[operonID]['crisprid']   ### Its a list
                targetRepeatSeqs = operonCollection[operonID]['crisprseq']   ### also a list, length equal to crisprIDs, index is the same
                for lindex in range(0,len(crisprIDs)):
                    unitseq = targetRepeatSeqs[lindex]
                    topalign = pairwise2.align.globalms(repeatSeq,unitseq.replace("U","T"),10,0,-10,-0.5,one_alignment_only=True)
                    formatalign = pairwise2.format_alignment(*topalign[0])
                    similarity = formatalign.split("\n")[1].count("|")/len(topalign[0][0].lstrip("-").rstrip("-"))
                    if similarity >= inrident:
                        acceptCRISPR.append(crisprIDs[lindex])
                        acceptCRISPRorientation[crisprIDs[lindex]] = "P"
                    else:
                        rcunitseq = reverse_complete(unitseq)
                        topalign = pairwise2.align.globalms(repeatSeq,rcunitseq.replace("U","T"),10,0,-10,-0.5,one_alignment_only=True)
                        formatalign = pairwise2.format_alignment(*topalign[0])
                        similarity = formatalign.split("\n")[1].count("|")/len(topalign[0][0].lstrip("-").rstrip("-"))
                        if similarity >= inrident:
                            acceptCRISPR.append(crisprIDs[lindex])
                            acceptCRISPRorientation[crisprIDs[lindex]] = "N"
    ### Write into spacer file, merge with raw spacer file
    dupbox = []
    with open(outfile,'w') as ff:
        for records in SeqIO.parse(rawspacerfile,'fasta'):
            if not str(records.seq) in dupbox and not reverse_complete(str(records.seq)) in dupbox:
                ff.write(">"+str(records.id)+"\n"+str(records.seq)+"\n")
                dupbox.append(str(records.seq))
        for crisprID in acceptCRISPR:
            if crisprID in spacerCollection:
                for keys, values in spacerCollection[crisprID].items():
                    if not str(values) in dupbox and not reverse_complete(str(values)) in dupbox:
                        if acceptCRISPRorientation[crisprID] == "P":
                            ff.write(">"+keys+"#P\n"+values+"\n")
                            dupbox.append(values)
                        else:
                            ff.write(">"+keys+"#N\n"+reverse_complete(values)+"\n")
                            dupbox.append(values)
    return True

def run_cas_blastp(proteinFile,casDB,CasOutput):
    casDB = os.path.abspath(casDB)
    cmd = "blastp -query "+proteinFile+" -db "+casDB+" -out "+CasOutput+" -outfmt '6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qcovs qlen slen'"
    os.system(cmd)
    return True
    ...
    with open(inraw,'r') as fa, open(insig,'w') as fb:
        for faline in fa.readlines():
            if not faline.strip() == "":
                ...
                evalue = float(faline.strip().split("\t")[-3])
                querystartpos = int(faline.strip().split("\t")[6])
                queryendpos = int(faline.strip().split("\t")[7])
                qlen = int(faline.strip().split("\t")[-1])
                qcovs = float((queryendpos-querystartpos+1)/qlen)
                if evalue <= 1 and qcovs == 1 and mismatches <= 3 and gapnumbers == 0:
                    ...
                    judgeA = querystartpos == 1 and queryendpos == qlen-1
                    judgeB = querystartpos == 2 and queryendpos == qlen
                    judgeC = querystartpos == 2 and queryendpos == qlen-1
                    if judgeA or judgeB or judgeC:
...
def get_genome_seqdump_files(intab, genomefile, outfile):
    dupbox = []
    refseqs = Fasta(genomefile)
    with open(intab,'r') as fa, open(outfile,'w') as fb:
        for faline in fa.readlines():
            sseqid = faline.strip().split("\t")[1]
            if sseqid in refseqs.keys():
                if not sseqid in dupbox:
                    fb.write(">"+sseqid+"\n"+str(refseqs[sseqid][::])+"\n")
                    dupbox.append(sseqid)
    return True
